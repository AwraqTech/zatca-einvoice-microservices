...
exports.invoiceTaxGenerationPdf = invoiceTaxGenerationPdf;
const pdfkit_1 = __importDefault(require("pdfkit"));
const axios_1 = __importDefault(require("axios"));
function invoiceTaxGenerationPdf(data, qrcode) {
    return __awaiter(this, void 0, void 0, function* () {
        const tempDoc = new pdfkit_1.default({ margin: 10 });
        let totalHeight = 0;
        ...
        const doc = new pdfkit_1.default({ font: './src/assets/fonts/IBMPlexSansArabic-Medium.ttf', size: [219, totalHeight], margin: 10 });
        ...
        columnsArabic.forEach((col, index) => {
            ...
            if (col === 'المنتج')
                columnWidth = columnWidths.productName;
            if (col === 'الكمية')
                columnWidth = columnWidths.quantity;
        ...
        lineData.forEach((row) => {
            ...
            rowValues.forEach((value, colIndex) => {
                let columnWidth = 0;
                if (columnsArabic[colIndex] === 'الإجمالي')
                    columnWidth = columnWidths.total;
                if (columnsArabic[colIndex] === 'السعر')
                    columnWidth = columnWidths.price;
                if (columnsArabic[colIndex] === 'المنتج')
                    columnWidth = columnWidths.productName;
                if (columnsArabic[colIndex] === 'الكمية')
                    columnWidth = columnWidths.quantity;

...
class API {
    ...
    compliance(certificate, secret) {
        ...
        const issueCertificate = async (csr, otp) => {
            ...
            const response = await axios_1.default.post(`${settings.SANDBOX_BASEURL}/compliance`, { csr: Buffer.from(csr).toString("base64") }, { headers: { ...auth_headers, ...headers } });
        ...
        const checkInvoiceCompliance = async (signed_xml_string, invoice_hash, egs_uuid) => {
            ...
            const response = await axios_1.default.post(`${settings.SANDBOX_BASEURL}/compliance/invoices`, {
    ...
    production(certificate, secret) {
        ...
        const issueCertificate = async (compliance_request_id) => {
            ...
            const response = await axios_1.default.post(`${settings.SANDBOX_BASEURL}/production/csids`, { compliance_request_id: compliance_request_id }, { headers: { ...auth_headers, ...headers } });
            if (response.status != 200)
        ...
        const reportInvoice = async (signed_xml_string, invoice_hash, egs_uuid) => {
            ...
            const response = await axios_1.default.post(`${settings.SANDBOX_BASEURL}/invoices/reporting/single`, {
                invoiceHash: invoice_hash,
                uuid: egs_uuid,
                invoice: Buffer.from(signed_xml_string).toString("base64")
            }, { headers: { ...auth_headers, ...headers } });
